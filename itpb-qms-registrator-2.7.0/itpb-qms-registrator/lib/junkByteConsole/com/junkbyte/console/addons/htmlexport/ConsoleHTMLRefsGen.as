	...
	public class ConsoleHTMLRefsGen
	{
		...
		public function fillData(data:Object):void
		{
			...
			while(line)
			{
				...
				processRefIdsFromLine(line.text,0);
				line = line.prev;
			}
		...
		private function processRefIdsFromLine(line:String, currentDepth:uint = 0):void
		{
			...
			while(result != null)
			{
				...
				// Added try catch here to avoid the log breaking when some object can not be proccesed
				try
				{
				...
				{
					trace("Error processing object !",id);
				}
		...
		private function processRef(obj:Object, currentDepth:uint):Object
		{
			...
			// fix for the missing constructor bug
			try
			{
