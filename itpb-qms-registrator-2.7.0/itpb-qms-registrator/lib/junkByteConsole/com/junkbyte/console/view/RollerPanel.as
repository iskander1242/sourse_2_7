			...
			for each(var child:DisplayObject in objs){
				...
				for (var i:uint=0; i<len; i++){
					...
					if(stepMap[obj] == undefined){
						...
						var isTranslatable:Boolean = false;
						
						// added Alexander Hadjimitov
						// to show ids for flex components
						if(Object(obj).hasOwnProperty("id") 
							&& Object(obj).id
							&& String(Object(obj).id).charAt(0) != "_" ){
							n = Object(obj).id;
							isSkinPart = true;
						}else if(Object(obj).hasOwnProperty("id")){
						...
						var translationKeys:Array = [];
						
						if(obj.hasOwnProperty("translationKeys")){
							isTranslatable = true;
							translationKeys = Object(obj).translationKeys
						}else if(obj.hasOwnProperty("id") 
							&& Object(obj).id
							&& obj.name 
							&& obj.name.charAt(0) != "_"
							&& obj.name.search(numberRegEX) < 0 ){

							isTranslatable = true;
							translationKeys.push(obj.name);
						}
						...
						if(isTranslatable){
							//str += "<a href='event:cl_"+ref+"'> [ Translations ]</a> "
							str += " Keys: [ " + translationKeys + " ]";
						}
